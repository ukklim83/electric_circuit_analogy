...
def main():
    ...
    inlet_edge_idx = [29, 30, 31]  # inlet edge indices
    inlet_node_idx = [23, 24, 25]  # inlet node indices
    inlet_dict = {
        inlet_node_idx[0]: inlet_edge_idx[0],
        inlet_node_idx[1]: inlet_edge_idx[1],
        inlet_node_idx[2]: inlet_edge_idx[2],
    }  # inlet connection dictionary

    changing_edges = [i for i in range(32)]
    ...
    ini_list = [
        ...
        ["f", 2, np.nan, "o", "m^3/s"],
        ["f", 3, np.nan, "o", "m^3/s"],
        ["f", 23, 6.6 * 10**(-6) / 60, "i", "m^3/s"],
        ["f", 24, 6.6 * 10**(-6) / 60, "i", "m^3/s"],
        ["f", 25, 6.6 * 10**(-6) / 60, "i", "m^3/s"],
    ]
